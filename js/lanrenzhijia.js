...
(function($) {
	...
	jQuery.fn.setLightBox = function(options) {
		...
		alertdiv.css({
			'width': alertdivWidth,
			'height': alertdivHeight
		}).html('').append(jQuery(this));
		if(!options.multil) {
		...
		var stop = clientH / 2 + document.documentElement.scrollTop + document.body.scrollTop;
		if(stop + mtop < 0) {
			stop = 20;
			mtop = 0;
		}
		...
		jQuery(this).add(jQuery(this).find('img')).load(function() {
			...
			stop = clientH / 2 + document.documentElement.scrollTop + document.body.scrollTop;
	...
	$.ml = {
		butShowTab: function(buts, tabs, tabs2, num, targe, backfn, startfn, speed) {
			if(num >= 0) {
			...
			$(buts.join()).each(function(i) {
				eval('$(this).' + targe + '(function(){' + 'if(!startfn||(startfn(i)!=false)){' + 'if(speed!=0){$(tabs.join()).stop(null,true).fadeTo(speed,0.1,function(){$(tabs.join()).css("display","none");$(tabs.join()).eq(i).css("display","block");});' + '$(tabs.join()).eq(i).stop(null,true).fadeTo(speed,1,function(){backfn?backfn(i):";"});}else{' + '$(tabs.join()).css("display","none");' + '$(tabs.join()).eq(i).css("display","block");' + 'backfn?backfn(i):";"}' + '}});');
				eval('$(this).' + targe + '(function(){' + 'if(!startfn||(startfn(i)!=false)){' + 'if(speed!=0){$(tabs2.join()).stop(null,true).fadeTo(speed,0.1,function(){$(tabs2.join()).css("display","none");$(tabs2.join()).eq(i).css("display","block");});' + '$(tabs2.join()).eq(i).stop(null,true).fadeTo(speed,1,function(){backfn?backfn(i):";"});}else{' + '$(tabs2.join()).css("display","none");' + '$(tabs2.join()).eq(i).css("display","block");' + 'backfn?backfn(i):";"}' + '}});');
			});
		},
...
(function($) {
	$.fn.showPartImg = function(options) {
		...
		img.parent().find('.bigimgdiv img').load(function() {
			...
			img.parent().find('.bigimgmove').css({
				'opacity': '0.6',
				'cursor': 'pointer'
			});
...
$(document).ready(function() {
	PointsPlus.init();
	IntroductionInteration.initHover();
	horizontalSwitcher();
	hrpagechange();
	milestonPageAction();
	productShare();
	...
	if($('.add-plus')[0] != undefined) {
		$('.add-plus').hover(function() {
			$(this).css({
				'border': '1px solid #E0E0E0',
				'background': '#EFEFEF'
			});
			$(this).find('.add-plus-one').css({
				'display': 'block'
			});
		}, function() {
			$(this).css({
...
function validateform() {
	...
	$(".required-input").each(function() {
		...
		if(!$("#demand").hasClass("active")) isValidated = false;
	});
	if(!isValidated) {
		alert('请确保要求填写的项目不为空');
		return isValidated;
	...
	if(!emailreg.test(email)) {
		alert('请填写正确的E-mail以便我们的客服人员联系您');
		return false;
	}
}

function moduleContact() {
	$("#demand").focus(function() {
		var pro = $(this).val();
	...
	$("#demand").blur(function() {
		var text = "请您在此填写需求，我们将尽快给予您答复。";
		if($(this).val() == "") {
			$(this).val(text);
			$(this).removeClass("active");
		}
	});
	$("#message-online").click(function() {
		$(".fiexd-hover").fadeTo("slow", 0.5);
		$(".fixed-message-frame").animate({
			height: 490
		}, 700);
	});
	$(".closefixed,#l-reset").click(function() {
		$(".fiexd-hover").fadeOut();
		$(".fixed-message-frame").animate({
			height: 0
		}, 700);
...
function productShare() {
	$(".product-share-frame").hover(function() {
		$(".add-plus-proudct").show();
		$(".add-plus-proudct").css("height", "150px");
		$(".add-plus-proudct a").show();
	}, function() {
		$(".add-plus-proudct").hide();
		$(".add-plus-proudct").css("height", "50px");
	});
}

function hrpagechange() {
	var buts1 = ['.jobitem'];
	var tabs1 = ['.hrbannerimg'];
	var tabs2 = ['.hr-desc'];

	function backfn1(i) {
		var key = jQuery('.hr-memu .active').attr('key');
		jQuery(buts1.join()).removeClass('active active' + key);
		jQuery(buts1.join()).eq(i).addClass('active active' + i);
	}
	jQuery.ml.butShowTab(buts1, tabs1, tabs2, -1, 'click', backfn1, '', 0);
	jQuery('.jobitem').each(function() {
		jQuery(this).click(function() {
			jQuery('#job').val(jQuery(this).attr('title'));
		})
	});
}

function doShare(ele, stype) {
	var sitetitle = '木兰设计';
	var stitle = $(ele).attr("stitle") + ' - ' + sitetitle;
	var simg = $(ele).attr("simg");
	var surl = document.getElementById('baseurl').value;
	var annuncement_link = $('#collect_' + parseInt($(ele).attr("sid"))).attr('href');
	var product_link = $("#product_link").attr("href");
	if(typeof(annuncement_link) != 'undefined') {
	...
	switch(stype) {
		...
		case "sina":
			...
			break;
		case "tqq":
			window.open('http://www.jiathis.com/send/?webid=tqq&url=' + u + '&title=' + t + (p != 'undefined' && p != undefined ? '&pic=' + p : ''), '_blank');
			x
			break;
		case "qzone":
...
var PointsPlus = {
	setting: {
		hover_animate_time: 200,
		point_animate_interval: 3000,
		point_animate_hide_time: 200,
		point_animate_show_time: 30,
		point_animate_stop: false,
		cur_hover_point: null
	},
	init: function() {
		if($('.service-point')[0] != undefined) {
			this.init_points();
			this.init_points_animate();
		}
	},
	init_points: function() {
		var point_showed = 0;
		$('.service-point').each(function() {
			point_showed++;
			PointsPlus.hide_show_point(this, 150 * point_showed);
		});
	},
	hide_show_point: function(ele, delay) {
		if(PointsPlus.setting.point_animate_stop == false && PointsPlus.setting.cur_hover_point == null || $(ele).attr('id') != PointsPlus.setting.cur_hover_point) {
			$(ele).delay(delay).animate({
				opacity: 0.4
			}, PointsPlus.setting.point_animate_hide_time, function() {
				if(PointsPlus.setting.point_animate_stop == false && PointsPlus.setting.cur_hover_point == null || $(ele).attr('id') != PointsPlus.setting.cur_hover_point) {
					$(this).animate({
						opacity: 1
					}, PointsPlus.setting.point_animate_show_time, function() {
						if(PointsPlus.setting.point_animate_stop == false && PointsPlus.setting.cur_hover_point == null || $(ele).attr('id') != PointsPlus.setting.cur_hover_point) {
							$(this).delay(200).animate({
								...
								if(PointsPlus.setting.point_animate_stop == false && PointsPlus.setting.cur_hover_point == null || $(ele).attr('id') != PointsPlus.setting.cur_hover_point) {
									$(this).animate({
										opacity: 1
									}, PointsPlus.setting.point_animate_show_time);
								} else {
									$(this).css({
										opacity: 1
									});
		...
		setTimeout(function() {
			PointsPlus.hide_show_point(ele, delay);
		}, PointsPlus.setting.point_animate_interval);
	},
	init_points_animate: function() {
		$('.service-point').hover(function() {
			PointsPlus.setting.point_animate_stop = true;
			$(this).stop().css({
				opacity: 1
			});
			PointsPlus.setting.cur_hover_point = $(this).attr('id');
			var posw = $(this).attr('posw');
			var pos = $(this).attr('pos2').split(',');
			$(this).css({
				'left': pos[0] + 'px',
				'top': pos[1] + 'px',
				'width': posw + 'px',
			...
			if($(this).attr('class').indexOf('point-left') > 0) {
				$(this).find('span').css({
					'display': 'block',
					'right': '-10px'
				}).stop().animate({
					opacity: 1,
					right: '0px'
				}, PointsPlus.setting.hover_animate_time);
				$(this).find('p').css({
					'display': 'block',
					'left': '-10px'
				}).stop().animate({
			...
			PointsPlus.setting.point_animate_stop = false;
			var pos = $(this).attr('pos1').split(',');
			var point_id = $(this).attr('id');
			if($(this).attr('class').indexOf('point-left') > 0) {
				$(this).find('span').stop().animate({
					opacity: 0,
					right: '-10px'
				}, PointsPlus.setting.hover_animate_time, function() {
					$(this).css({
						'display': 'none'
					});
				});
...
var IntroductionInteration = {
	setting: {
		movieObj: null,
		movieTime: 400
	},
	initHover: function() {
		if($('.introduction-hover')[0] != undefined) {
			$('.introduction-hover').hover(function() {
				if(IntroductionInteration.setting.movieObj != undefined) {
					IntroductionInteration.setting.movieObj.stop();
				}
				IntroductionInteration.setting.movieObj = tween($('#introduction-hover-bg').css('left'), (parseInt($(this).attr('id')) * 313 + 1) + 'px', IntroductionInteration.setting.movieTime / 1000, 'easeoutelastic');
				IntroductionInteration.setting.movieObj.run = function(ps) {
					$('#introduction-hover-bg').css('left', ps);
				}
			}, function() {
				return false;
			});
...
function milestonPageAction() {
	...
	$(".year-tab").each(function() {
		$(this).bind("click", function() {
			var year = $(this).html();
			$(".command-bar").find(".active").removeClass("active");
			$(this).addClass("active");
			$(".milestone-list").find(".active").removeClass("active");
			$(".year-frame").hide();
			$(".m-list-frame").removeClass("block");
			$(".m-list-frame").addClass("none");
			$(".m-frame" + year).addClass("block");
			$(".m-frame" + year).find(".scrollcontent").css("top", "0");
			$(".m-frame" + year).find(".scroll-bar").css("top", "12px");
			$(".year" + year).fadeIn(500);
			$(".year" + year).addClass("active");
			$(".year" + year + " li:first-child").click();
		});
	});
	$(".milestone-item").each(function() {
		$(this).bind("click", function() {
			var barMoved = $(this).parent().parent().css('top');
			barMoved = parseInt(barMoved.match(/^(-?\d+)/)[0], 10);
			var thisPosition = $(this).attr("position");
			$("#pointer-left").animate().stop();
			$("#pointer-left").animate({
				top: 67 * thisPosition + 17 + barMoved
			}, 400);
			$(".milestone-item").removeClass("active");
			$(this).addClass("active");
			$("#current-title").html($(this).find(".info-title").html());
			$("#current-date").html($(this).find(".info-date").attr("title"));
			$("#current-intro").html($(this).find(".info-intro").html());
			$("#current-img").html($(this).find(".info-img").html());
		});
	});
}
$(document).ready(function() {
		...
		switch(moveDirecrion) {
			case '0':
				...
				e.animate({
					...
					duration: moveInSpeed,
					easing: moveInType
				});
				break;
			case '1':
				e.css("left", "0").css("top", "-260px");
				e.animate({
					"top": "0",
					"opacity": "1"
				}, {
				...
				break;
			case '2':
				e.css("left", "460px").css("top", "-260px");
				e.animate({
					"top": "100px",
					"left": "0",
					"opacity": "1"
			...
			case '5':
				...
				break;
			case '6':
				e.css("left", "-260px").css("top", "460px");
				e.animate({
					"left": "0",
					"top": "0",
					"opacity": "1"
			...
			case '7':
				...
				break;
			case '8':
				e.css("left", "-260px").css("top", "-260px");
				e.animate({
					"left": "0",
					"top": "0",
					"opacity": "1"
...
function initalTopAddsAnimate() {
	var isChecked = getCookie("checkedTopAdds");
	var showTime = parseInt($("#t-showtime").val());
	var openspeed = parseInt($("#t-openspeed").val());
	var closespeed = parseInt($("#t-closespeed").val());
	var autoclose = parseInt($("#t-autoclose").val()) == 0 ? false : true;
	var showType = $("#t-showType").val();
	var $moveAbleFrame = $("#top-add-show");
	var $closeButton = $("#top-add-close");
	var $mainImg = $("#add-img");
	var frameHeight = $("#add-img").height();
	var $showButton = $("#get-adds-blinking");
	$closeButton.css("opacity", "0");
	$mainImg.show();

	function showButtonBlinking() {
		$showButton.animate({
			opacity: 1
		}, 2000, function() {
			setTimeout(showButtonBinkingOut, 1000);
		});
	}
	...
	function showButtonBinkingOut() {
		...
		setTimeout(showButtonBlinking, 3000);
	}
	showButtonBlinking();
	$showButton.bind("click", function() {
		if(showType == 'slideDown') {
	...
	function slideDownAnimate(frameHeight) {
		$mainImg.show();
		$moveAbleFrame.css("width", "100%").css("z-index", "99999").css("position", "fixed").css("opacity", "0").css("left", "0");
		$moveAbleFrame.animate({
			height: frameHeight,
			opacity: 1
		}, openspeed, function() {
			if(autoclose) {
	...
	function fadeInAnimate(frameHeight) {
		$mainImg.show();
		$moveAbleFrame.css("width", "100%").css("z-index", "99999").css("position", "fixed").css("opacity", "0").css("height", frameHeight).css("left", "0");
		$moveAbleFrame.animate({
			opacity: 1
		}, openspeed, function() {
			if(autoclose) {
				...
				function closeBlinking() {
					setTimeout(closeBlinkingOut, 1000);
				}
...
function horizontalSwitcher() {
	if(!$('.horizontal-switcher').isExist()) {
		return;
	}
	$(".horizontal-switcher").each(function() {
		var thisSwitcher = $(this);
		var items = $(this).find(".item");
		var itemWidth = items.outerWidth();
		var barWidth = items.length * itemWidth;
		var moveableBar = $(this).find(".moveable");
		moveableBar.width(barWidth);
		var leftButton = $(this).find(".left-button");
		var rightButton = $(this).find(".right-button");
		switcher(leftButton, rightButton, moveableBar, items, itemWidth, thisSwitcher);
	});
}

function switcher(leftButton, rightButton, moveableBar, items, itemWidth, thisSwitcher) {
	var showCount = thisSwitcher.attr("show");
	var fadein = thisSwitcher.attr("fadein");
	var pagebt = thisSwitcher.attr("pagebt");
	var itemCount = items.length;
	var currentPoninter = 0;
	leftButton.css("opacity", 0);
	rightButton.addClass("cursor-pointer");
	rightButton.bind("click", function() {
		var action = 0;
		if((currentPoninter + 3) < itemCount) {
			currentPoninter++;
			moveableBar.animate().stop();
			moveableBar.animate({
				left: -itemWidth * currentPoninter
			}, 300, function() {
				if(currentPoninter + 3 == itemCount) {
					leftButton.addClass("cursor-pointer");
					leftButton.fadeTo(300, 1);
					rightButton.removeClass("cursor-pointer");
					rightButton.fadeTo(300, 0);
				}
			});
			if(fadein == 1) {
				var preItem = moveableBar.find(".item:eq(" + (currentPoninter - 1) + ")");
				var endItem = moveableBar.find(".item:eq(" + (currentPoninter + 2) + ")");
				endItem.css("opacity", 0);
				preItem.fadeTo(300, 0);
				endItem.fadeTo(800, 1);
			}
		}
	});
	leftButton.bind("click", function() {
		if(currentPoninter >= 1) {
			currentPoninter--;
			moveableBar.animate().stop();
			moveableBar.animate({
				left: -itemWidth * currentPoninter
			}, 300, function() {
				if(currentPoninter == 0) {
					leftButton.removeClass("cursor-pointer");
					leftButton.fadeTo(300, 0);
					rightButton.addClass("cursor-pointer");
					rightButton.fadeTo(300, 1);
				}
			});
			if(fadein == 1) {
				var preItem = moveableBar.find(".item:eq(" + (currentPoninter) + ")");
				var endItem = moveableBar.find(".item:eq(" + (currentPoninter + 3) + ")");
				endItem.fadeTo(300, 0);
				preItem.fadeTo(800, 1);
			}
		}
	});
	if(pagebt == 1 && thisSwitcher.find(".bigpagenation").isExist()) {
		var pageButton = thisSwitcher.find(".pagenation-b");
		var lastLose = itemCount % showCount != 0 ? showCount - itemCount % showCount : 0;
		pageButton.bind("click", function() {
			pageButton.removeClass("active");
			$(this).addClass("active");
			var gotoPage = $(this).attr("title");
			var pointer = (gotoPage - 1) * 3;
			if(gotoPage != pageButton.length) {
				currentPoninter = pointer;
				moveableBar.animate({
					left: -itemWidth * pointer
				}, 500);
			} else {
				currentPoninter = pointer - lastLose;
				moveableBar.animate({
					left: -itemWidth * (pointer - lastLose)
				}, 500);
			}
		});
	}
